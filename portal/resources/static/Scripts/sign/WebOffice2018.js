function WebOffice2015() {
    ...
    this.DocTypeValue = {DOC: 0, DOCX: 16, XLS: 56, XLSX: 51, WPS: 0, ET: 56}; //枚举
    所有的文档类型值用户保存用

    this.DocSuffixType = {
        0: ".doc", 12: ".docx", 56: ".xls", 51: ".xlsx", 0: ".wps",
        56: ".et"
    }; //根据打开文档类型，来获取后缀名称


    // this.DocTypeValue = { DOC: 0, DOCX: 16, XLS: 56, XLSX: 51 }; //枚举所有的文档类型
    值用户保存用

    // this.DocSuffixType = { 0: ".doc", 12: ".docx", 56: ".xls", 51: ".xlsx" }; //根据
    打开文档类型，来获取后缀名称

    this.setVersion = -1;
    this.OfficeVersion = {v2003: 1, vOther: 0}  //判断Office版本，这个只要用于保存到本
    地
    ...
    */
    ...
    this.HookEnabled = function () {//解决在Firefox和chrome浏览器下调用iWebPlugin崩溃崩
        溃的问题需要在load()
        函数下加如下代码
    ...
    this.getOpenDocSuffix = function (fileType) {
        ...
        var exts;
        exts = "";
        if (this.isWPS() || !this.getOfficeVersion()) {//如果是office2003是不支持x
            格式的文档
        ...
        exts += ")|*" + fileType
        if (this.isWPS() || !this.getOfficeVersion()) {//如果是office2003是不支持x格式的文
            档
            exts += ";*" + fileType + "x";
        }
        exts += "|";
    ...
    this.setShowDialog = function (thisType, exts) {
        switch (thisType) {
            ...
            case this.ShowDialog.DialogSaveAs: //另存为
                //   this.blnIE() ? this.obj.ShowDialog(thisType) :
                this.obj.ExecuteScript(thisType + "", "ActiveObject.ShowDialog(" + thisType + ")");
                break; //打开窗口
    ...
    this.WebClose = function () { //多浏览器调用关闭
        if (navigator.userAgent.indexOf("Firefox") >= 0) {//卸载时候，火狐不能使用close方
            法
            return;
        }
        this.Close();
    ...
    this.WebOpen = function (mBoolean) {
        ...
        httpclient.AddForm("FormData", this.WebSendMessage()); //这里是自定义json 传输格
        式。
    ...
    this.WebSaveLocalFile = function (FileName) {
        ...
        this.Save(FileName, this.getOfficeVersion(), this.FileType.substring
        (1).toUpperCase());
    }
    ...
    this.SAVEFILE = function (httpclient, FileName) {//时发生交互OPTION值为：SAVEFILE
        ...
        if (httpclient.Open(this.HttpMethod.Post, this.WebUrl, false)) {//这里采用同步方
            式打开文档。我要返回值
            if (!httpclient.Send()) {
    ...
    this.LOADFILE = function (httpclient) { //Http 下载文档
        ...
        if (httpclient.Open(this.HttpMethod.Post, this.WebUrl, false)) {//这里采用同步方
            式打开文档。我要返回值
            if (httpclient.Send()) {
                if (httpclient.GetResponseHeader("MsgError") == "404") { //判断服务器是
                    否存在文件
                    this.CreateFile();
                ...
                if (this.hiddenSaveLocal(httpclient, this, false, false)) {//下载成功时
                    if (this.OpenLocalFile(this.FilePath + this.FileName)
                        == 0) { //打开本地磁盘文件
                        this.getOfficeVersion();//打开文档后，判断当前
                        office版本
                        return true;
                    } else {
                        var windows = window.confirm("打开文档错误，可
                        能当前编辑软件不支持当前格式！\r\r单击“确定”关闭。单击“取消”继续。");
                        if (windows == 1) {
                            window.close();
                        }
                        return false;
    ...
    this.hiddenSaveLocal = function (httpclient, webOffice, isHidden, isInsertFile,
                                     OtherName) {
        if (isHidden) {
    ...
    this.getOfficeVersion = function () {
        ...
        try {
            if (this.setVersion == -1) {
                getVersion = parseFloat
                (this.obj.ActiveDocument.Application.Version);
    ...
    this.WebPageCode = function () {
        var FunctionString = "ActiveObject.ActiveDocument.Application.Dialogs
        (294).Show()
        ";
    ...
    /*ExecuteScript可将同步函数转换为异步，解决在FireFox、Chrome浏览器中有弹框或运行时间过
长导致弹出不响应提示框的问题。*/
    ...
    */
    ...
    this.WebSetProtect = function (Boolean, password) {
        ...
        Boolean ? this.VBAProtectDocument(docType, password) :
            this.VBAUnProtectDocument(docType, password);
    }
    ...
    this.WebAddBookMarks = function (BMarksName, BMarksValue) {
        if (!this.obj.ActiveDocument.BookMarks.Exists(BMarksName)) {//判断是否存在该
            书签
            var BMVLength = BMarksValue.length;
            this.obj.ActiveDocument.Application.Selection.TypeText(BMarksValue);//插
            入内容
            this.obj.ActiveDocument.Application.Selection.MoveLeft(Unit = 1, Count =
                BMVLength);
    ...
    this.BookMark = "";
    this.ImageName = "";
    this.WebInsertImage = function (BookMark, ImageName) {//插入远程图片：BookMark插入的
        书签的位置;
        ImageName远程图片的名称
        ...
        httpclient.ShowProgressUI = false;//隐藏进度条
        if (httpclient.Open(this.HttpMethod.Post, URL + "/Document/" + ImageName,
            true)) {
            httpclient.Send();
            httpclient.Clear();
        }
    ...
    this.InsertImageByBookMark = function () {
        if (this.BookMark == null || this.BookMark == 'null' || this.BookMark == "") {
            this.obj.ActiveDocument.Application.Selection.GoTo(-1, 0, 0,
                this.BookMark);
        }
        this.obj.ActiveDocument.Application.Selection.InlineShapes.AddPicture
        (this.FilePath + this.ImageName);
        this.obj.ActiveDocument.InlineShapes.Item(1).ConvertToShape();   //转为浮动型
        this.obj.ActiveDocument.Shapes.Item(1).WrapFormat.Type = 5;     //0:四周型  1：
        紧密型
        2：穿越型环绕
        3：浮于文字上方
        4：上下型环绕
    ...
    this.DownloadToFile = function (DownFileName, SavePath) {
        ...
        var URL = this.WebUrl.substring(0, this.WebUrl.lastIndexOf("/"));
        httpclient.ShowProgressUI = false;//隐藏进度条
        if (httpclient.Open(this.HttpMethod.Get, URL + "/Document/" + DownFileName,
            false)) {//指定下载模板的名称
            if (httpclient.Send()) {
                if (httpclient.Status == 200) {
    ...
    /*判断浏览器类型*/
    this.blnIE = function () {
        return (window.ActiveXObject != undefined) || (window.ActiveXObject != null) ||
            ("ActiveXObject" in window)
    }
    /*判断是否是WPS*/
    this.isWPS = function () {
        return this.FileType.toUpperCase() == ".WPS" || this.FileType.toUpperCase()
            == ".ET";
    }
    /*
    * 控件版本
    */
    ...
    this.VBAProtectDocument = function (docType, password) {
        ...
        if (docType == this.DocType.EXECL) {//word 保护模式，这里只保护表单1的其他的安自
            己需求编写
            if (!this.obj.ActiveDocument.Application.Sheets(1).ProtectContents) { //
                判断表单是否是保护的
    ...
    this.VBAInsertFile = function (Position, FileName) {
        try {
            ...
            this.obj.ActiveDocument.Application.Selection.InsertFile(FileName, "",
                false, false, false);
            return true;
        } catch (e) {
            return false;
    ...
    this.WebAreaProtect = function (BMarksName) {
        if (!this.obj.ActiveDocument.BookMarks.Exists(BMarksName)) {//判断是否存在该书
            签
        ...
        range = this.obj.ActiveDocument.Bookmarks(BMarksName).Range;
        range.Select();
        this.obj.ActiveDocument.bookmarks(BMarksName).range.editors.add(-1); //常量：
        wdeditoreveryone = -1
        this.obj.ActiveDocument.Protect(3, false, "123", false, false);//常量：
        wdAllowOnlyReading = 3
        this.obj.ActiveDocument.Application.Selection.MoveLeft(Unit = 1, Count = 1);
        this.obj.ActiveDocument.ActiveWindow.View.ShadeEditableRanges = false;//取消"突
        出显示可编辑区域
        "
    }
    //取消区域保护
    this.WebAreaUnprotect = function (BMarksName) {
        if (this.obj.ActiveDocument.BookMarks.Exists(BMarksName)) {//判断是否存在
            该书签
            try {
                ...
                range = this.obj.ActiveDocument.Bookmarks(BMarksName).Range;
                range.Select();//选定书签内容
                this.obj.ActiveDocument.DeleteAllEditableRanges(-1); //去掉突出
                显示
            } catch (e) {
            }
        } else {
    ...
    this.AddToolbar = function () {
        ...
        customtoolbar.AddToolButton(301, "手写签名", URL
            + "/HandWrite/" + "shouxie24.png", "手写签名", 0);
        customtoolbar.AddToolButton(302, "停止手写", URL
            + "/HandWrite/" + "tingzhi24.png", "停止手写", 0);
        customtoolbar.AddToolButton(303, "文字签名", URL
            + "/HandWrite/" + "wenzi24.png", "文字签名", 0);
        customtoolbar.AddToolButton(300, "-", "", "", 1);
        customtoolbar.AddToolButton(304, "图形签名", URL
            + "/HandWrite/" + "tuxing24.png", "图形签名", 0);
        customtoolbar.AddToolButton(305, "取消上一个签批", URL
            + "/HandWrite/" + "quxiao24.png", "取消上一个签批", 0);
        customtoolbar.AddToolButton(306, "显示用户签批", URL
            + "/HandWrite/" + "xianshi24.png", "显示用户签批", 0);
    }
    ...
    this.ShowCustomToolbar = function (bVal) {
        ...
        //if(bVal==true) this.obj.Style.CustomToolbarStartColor=0xDDA0DD; //设置签批工
        具栏颜色
    }
    /**
     ×××××××××××××××××××××××××××××××××××××××××××
    ...
    this.SaveAsPdf = function (FilePath) {
        ...
        if ((this.FileType == ".doc") || (this.FileType == ".docx") || (this.FileType
            == ".wps")) {
            try {
                //True  中间的0不包含书签 1转换之后会包含书签
                this.obj.ActiveDocument.ExportAsFixedFormat
                (FilePath, 17, false, 0, 0, 1, 1, 0, true, true, 1, true, true, true);
            } catch (e) {
    ...
    this.SaveAsHtml = function (FilePath) {
        if ((this.FileType == ".doc") || (this.FileType == ".docx") || (this.FileType
            == ".wps")) {
            try {
                var ret = this.obj.ActiveDocument.SaveAs
                (FilePath, 8, false, "", false, "", false, false, false, false, false, 0);
                this.obj.ActiveDocument.Application.ActiveWindow.View.type =
                    3;//3是页面视图
            } catch (e) {
