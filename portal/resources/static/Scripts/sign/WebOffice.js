...
//	   	};
    ...
    this.ShowMenuBar = function (mValue) {
        ...
        switch (mValue) {
            ...
            case false:
            case "0":
                style.ShowMenuBar = false;
                break; //隐藏菜单栏
            case 1:
            case "1":
            case true:
                style.ShowMenuBar = true;
                break; //显示菜单栏
            default:
                ;
                return;
    ...
    this.NewShowToolBar = function (mValue) {
        ...
        switch (mValue) {
            case false :
            case 0:
                style.ShowCustomToolbar = false;
                style.ShowToolBars = true;
                break; 		//自定义工具栏隐藏，office工具栏显示
            case true :
            case 1:
                style.ShowCustomToolbar = true;
                style.ShowToolBars = true;
                break; 		//自定义工具栏显示，office工具栏显示
            case 2:
                style.ShowCustomToolbar = false;
                style.ShowToolBars = false;
                break; 	//自定义工具栏隐藏，office工具栏隐藏
            case 3:
                style.ShowCustomToolbar = true;
                style.ShowToolBars = false;
                break; 		//自定义工具栏显示，office工具栏隐藏
        }
    }
    ...
    */
    ...
    this.OleDialogPrint = function (OLEFlag, bCancel) {
        if (OLEFlag == 1) {
            WebOffice.obj.ShowDialog(0);	//新建文档
        } else if (OLEFlag == 2) {
            var exts;
            ...
            exts += "|All File(*.*)|*.*";
            exts += "||";
            WebOffice.obj.ShowDialog(1, exts, 0);//打开本地文档(弹窗)
        } else if (OLEFlag == 4) {
            WebOffice.obj.ShowDialog(2);	//另存为本地文档
        } else if (OLEFlag == 8) {
            WebOffice.obj.ShowDialog(3);	//另存为拷贝
        } else if (OLEFlag == 16) {
            WebOffice.obj.ShowDialog(4);	//打印
        } else if (OLEFlag == 32) {
            WebOffice.obj.ShowDialog(5);	//打印设置
        } else if (OLEFlag == 64) {
            WebOffice.obj.ShowDialog(6);	//文档属性
        } else if (OLEFlag == 128) {
            WebOffice.obj.ActiveDocument.PrintPreview();	//打印预览
        }
    }
