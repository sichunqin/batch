...
if (!jSuites && typeof (require) === 'function') {
    ...
    require('Using/JExcel/css/jsuites.css');
}
...
var jexcel = (function (el, options) {
    ...
    var defaults = {
        ...
        text: {
            ...
            showingPage: '显示{0}到{1}',
            show: '显示 ',
            search: '搜索',
            entries: ' 条',
            columnName: '列名',
            insertANewColumnBefore: '之前插入一个新列',
            insertANewColumnAfter: '之后插入一个新列',
            deleteSelectedColumns: '删除所选列',
            renameThisColumn: '重命名此列',
            orderAscending: '升序',
            orderDescending: '降序',
            insertANewRowBefore: '之前插入一个新行',
            insertANewRowAfter: '之后插入一个新行',
            deleteSelectedRows: '删除选定的行',
            editComments: '编辑评论',
            addComments: '添加评论',
            comments: '评论',
            clearComments: '清除评论',
            ...
            saveAs: '另存为...',
            about: '关于',
            areYouSureToDeleteTheSelectedRows: '您确定要删除所选行吗?',
            areYouSureToDeleteTheSelectedColumns: '您确定要删除所选列?',
            thisActionWillDestroyAnyExistingMergedCellsAreYouSure: '此操作将销毁任何现有的合并单元格。 你确定吗?',
            thisActionWillClearYourSearchResultsAreYouSure: '此操作将清除您的搜索结果。 你确定吗?',
            thereIsAConflictWithAnotherMergedCell: '与另一个合并的单元格存在冲突',
    ...
    obj.createTable = function () {
        ...
        if (obj.options.firstColumnTitle == true)
            tempCol.style.display = "none";
    ...
    obj.createRow = function (j, data) {
        ...
        //swq 首行为标题调整样式
        if (j == 0 && obj.options.firstRowTitle == true) {
            obj.rows[j].style.display = 'none';
            if (obj.options.firstColumnTitle == true) {
                obj.thead.rows[0].children[0].innerHTML = data[0];
                obj.thead.rows[0].children[0].style.textAlign = "center";
            }
        }
        // Row number label
        ...
        for (i = 0; i < obj.options.columns.length; i++) {
            ...
            //swq 首行为标题行
            if (j == 0 && obj.options.firstRowTitle == true) {
                obj.thead.rows[0].children[i + 1].innerText = data[i];
                obj.thead.rows[0].children[i + 1].title = data[i];
            }
    ...
    obj.createCell = function (i, j, value) {
        ...
        if (obj.options.columns[i].field)
            td.setAttribute('field', obj.options.columns[i].field);
        ...
        if (obj.options.columns[i].type == 'hidden') {
            ...
            element.name = 'c' + i;
            element.checked = (value == 1 || value == true || value == 'true' || value == '是') ? true : false;
            ...
            td.classList.add('jexceldropdown');
        ...
        //增加提示配置
        if (obj.options.isCellPrompt) {
            td.classList.add('dxnJexcelTdPrompt');
            td.setAttribute("title", value);
        }

        //swq 首列样式调整
        if (i == 0 && obj.options.firstColumnTitle == true) {
            // var color = $(obj.headers[0]).css("background-color");
            // if (color) td.style.backgroundColor = color;
        }

        //swq 可配置列样式
        if (obj.options.columns[i].cellStyle) {
            var list = obj.options.columns[i].cellStyle.split(";");
            for (var i = 0; i < list.length; i++) {
                var item = list[i];
                if (item) {
                    var styles = item.split(':');
                    if (styles[1])
                        td.style[styles[0]] = styles[1];
                }
            }
        }
    ...
    obj.createCellHeader = function (colNumber) {
        ...
        if (obj.options.columns[colNumber].field)
            obj.headers[colNumber].setAttribute('field', obj.options.columns[colNumber].field);
        ...
        //swq 可配置头部样式
        if (obj.options.columns[colNumber].titleStyle) {
            var list = obj.options.columns[colNumber].titleStyle.split(";");
            for (var i = 0; i < list.length; i++) {
                var item = list[i];
                if (item) {
                    var styles = item.split(':');
                    if (styles[1])
                        obj.headers[colNumber].style[styles[0]] = styles[1];
                }
            }
        }
    }
    ...
    obj.openEditor = function (cell, empty, e) {
        ...
        if (!obj.ignoreEvents) {
            if (typeof (obj.options.oneditionstart) == 'function') {
                if (obj.options.oneditionstart(el, cell, x, y) == false)
                    return;
            }
        }
        ...
        if (obj.records[y][x].classList.contains('readonly') == true && !force) {
            ...
            if (obj.options.columns[x].editor) {
                ...
                if (obj.options.columns[x].type == 'checkbox' || obj.options.columns[x].type == 'radio') {
                    ...
                    if (value == undefined || value == "#ERROR") value = "";
                    obj.records[y][x].innerHTML = value;
            ...
            if (obj.options.isCellPrompt && obj.records[y][x]) {
                obj.records[y][x].setAttribute("title", value);
            }
        }
    ...
    obj.setStyle = function (o, k, v, force, ignoreHistoryAndEvents) {
        ...
        var applyStyle = function (cellId, key, value) {
            ...
            if (obj.records[cell[1]] && obj.records[cell[1]][cell[0]]) {
                ...
                if (obj.options.firstColumnTitle == true && key == "background-color") {
                    // var y = obj.records[cell[1]][cell[0]].getAttribute("data-y");
                    // var x = obj.records[cell[1]][cell[0]].getAttribute("data-x");
                    // if (parseInt(y) > 0 && parseInt(x) == 0)
                    //     value = currentValue
                }
                // Change layout
    ...
    obj.insertRow = function (mixed, rowNumber, insertBefore) {
        ...
        if (obj.options.allowInsertRow == true) {
            ...
            for (var row = rowIndex; row < (numOfRows + rowIndex); row++) {
                ...
                //swq新增是获取相邻的tr样式
                var styleRow = insertBefore ? currentRows[0] : obj.rows[rowNumber];
                if (styleRow && styleRow.children && tr.children) {
                    for (var col = 0; col < tr.children.length; col++) {
                        if (styleRow.children[col] && styleRow.children[col].style && tr.children[col])
                            tr.children[col].style.textAlign = styleRow.children[col].style.textAlign;
                    }
                }
    ...
    obj.copy = function (highlighted, delimiter, returnData) {
        ...
        // Reset container
        obj.style = [];
        if (obj.el.isReplaceMarks == null) obj.el.isReplaceMarks = true;
        ...
        for (var j = 0; j < y; j++) {
            ...
            for (var i = 0; i < x; i++) {
                ...
                if (!highlighted || obj.records[j][i].classList.contains('highlight')) {
                    // Values
                    var value = obj.options.data[j][i];
                    if (obj.el.isReplaceMarks && value.match && (value.match(/,/g) || value.match(/\n/) || value.match(/\"/))) {
    ...
    if (jexcel.current) {
        if (jexcel.current.edition) {
            ...
            if (jexcel.current) {
                ...
                if ((x || y) && jexcel.current.options.disableMenu != true) {
                    // Table found
