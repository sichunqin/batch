(function ($) {
    ...
    function init(target, filters) {
        ...
        if (opts.filterRules && opts.filterRules.length > 0) {
            ...
            if (target.defaultFilterRules) {
                $.map(target.defaultFilterRules, function (rule) {
                    addFilterRule(target, rule);
                });
            }
        }
        ...
        function initCss() {
            if (!$('#datagrid-filter-style').length) {
                $('head').append(
                    ...
                    '.datagrid-filter-row .textbox,.datagrid-filter-row .textbox .textbox-text{background: rgba(0,0,0,0)}' +
                    '.datagrid-filter-row input{margin:0;background: rgba(0,0,0,0)}' +
